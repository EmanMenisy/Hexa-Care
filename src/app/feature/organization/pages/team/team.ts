...
export class Team {
  ...
  getTableColumns(): void {
    this.tableColumns = [
      {
        field: 'name',
        name: 'organization.team.table.name',
        type: TableHeaderType.String,
        sortable: false,
      },
      {
        field: 'companyName',
        name: 'organization.team.table.companyName',
        type: TableHeaderType.String,
      },
      {
        field: 'branchName',
        name: 'organization.team.table.branchName',
        type: TableHeaderType.String,
      },
      {
        field: 'departmentName',
        name: 'organization.team.table.departmentName',
        type: TableHeaderType.String,
      },
      {
        field: 'createdAt',
        name: 'organization.team.table.createdAt',
  ...
  getAllList(pageNumber = 1) {
    ...
    this.organizationService.getAllTeams(request).subscribe({
      next: (res) => {
  ...
  onDelete(roleId: string) {
    this.organizationService.onDeleteTeam(roleId).subscribe({
      next: (res) => {
        console.log(res);
      },
      error: (err) => {},
